...
export default function PresensiContent() {
 ...
 const mahasiswaListRef = useRef([]);
 useEffect(() => {
  mahasiswaListRef.current = mahasiswaList;
 }, [mahasiswaList]);
  ...
  deteksiIntervalRef.current = setInterval(async () => {
   ...
   try {
    ...
    detections.forEach((det) => {
     ...
     mahasiswaListRef.current.forEach((mhs) => {
     ...
     // Buat persegi dari sisi terpanjang
     ...
     ctx.lineWidth = 2;
     ctx.strokeRect(sx, sy, size, size); // ← pakai size x size

     // Label di atas kotak persegi
     ctx.fillStyle = color;
